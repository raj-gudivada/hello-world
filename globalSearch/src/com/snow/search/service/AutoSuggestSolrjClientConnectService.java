...
public class AutoSuggestSolrjClientConnectService {
	...
	public String autoFillQuerySearch(RequestDTO requestDTO,AttributesDTO attributesDTO) throws IOException, SolrServerException {
		...
		/*}
			...
			logger.error("Error occured while querying.");

		}catch(SolrServerException solrServerException){
			logger.error("Error : Please check Solr log !");
		}
		catch (Exception e) {
			...
			ObjectMapper mapper = new ObjectMapper();
			try{
				String error = mapper.writeValueAsString(errorhandlerDTO);
				return error;
			}
...
*/		    solr.close();
	...
	public String autoSuggestTermsSearch(String queryParam,Integer maxRows,AttributesDTO attributes) throws IOException, SolrServerException{
