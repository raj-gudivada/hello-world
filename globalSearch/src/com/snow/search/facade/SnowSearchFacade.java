...
public String fetchUserRoles(RequestDTO reqDto) throws IOException{
    ...
    if((userRoles.isEmpty()) || (!userRoles.isEmpty()&& userRoles.size()==1&& userRoles.get(0).trim().equals(""))) {
			response =  null;
	}
    else {
...
public AttributesDTO fetchAll(RequestDTO reqDTO) throws IOException{
	...
	if(searchTypes.size()>1) {
		...
		if(facetselected.contains("")||facetselected.isEmpty()||facetselected.contains(" ")||facetselected==null) {
		...
		for (String eachSearchType : searchTypes) {
			...
			if(eachSearchType.isEmpty()||eachSearchType.equalsIgnoreCase("NULL")||eachSearchType.equalsIgnoreCase("NONE")||eachSearchType.equalsIgnoreCase("ALL")) {
				attributes.setFields(values.getProperty("basic.fl.all"));
				if(facetselected.contains("")||facetselected.isEmpty()||facetselected.contains(" ")||facetselected==null) {
					attributes.setFacetValue(values.getProperty("basic.all.facets"));
					attributes.setFacet(Boolean.TRUE);
			...
			if(!eachSearchType.isEmpty()&& eachSearchType.equalsIgnoreCase("SC")) {
				...
				attributes.setFilterType(values.getProperty("filter.type.all")+":"+values.getProperty("sc.filter.type"));
				if(facetselected.contains("")||facetselected.isEmpty()||facetselected.contains(" ")||facetselected==null) {
			...
			if(!eachSearchType.isEmpty()&& eachSearchType.equalsIgnoreCase("UD")) {
				...
				attributes.setFilterType(values.getProperty("filter.type.all")+":"+values.getProperty("ud.filter.type"));
				if(facetselected.contains("")||facetselected.isEmpty()||facetselected.contains(" ")||facetselected==null) {
			...
			if(!eachSearchType.isEmpty()&& eachSearchType.equalsIgnoreCase("KB")) {
				...
				attributes.setFilterType(values.getProperty("filter.type.all")+":"+values.getProperty("kb.filter.type"));
				if(facetselected.contains("")||facetselected.isEmpty()||facetselected.contains(" ")||facetselected==null) {
			...
			if(!eachSearchType.isEmpty()&& eachSearchType.equalsIgnoreCase("LOC")) {
				...
				attributes.setFilterType(values.getProperty("filter.type.all")+":"+values.getProperty("loc.filter.type"));
				attributes.setSpecialUser(fetchUserRoles( reqDTO));
...
public String getQuerySearch(RequestDTO requestDTO,String user,AttributesDTO attributes) throws IOException, SolrServerException {
	 SolrjClientConnectService solrjClientConnectService= new SolrjClientConnectService();
 	 String queryResponse=solrjClientConnectService.querySearch(requestDTO.getQueryParam(),requestDTO.getStart(),requestDTO.getRows(),user,attributes);
	return queryResponse;
	
}

/*public AttributesDTO fetchAutoFillFields(RequestDTO reqDto) throws IOException {
	...
	for (String searchValue : searchTypes) {
		if(searchValue.isEmpty()| searchValue.equalsIgnoreCase("NULL")|searchValue.equalsIgnoreCase("NONE")|searchValue.equalsIgnoreCase("ALL")) {
			...
			attributes.setFields(values.getProperty("autoFill.fl.all"));
		...
		if(!searchValue.isEmpty()&& searchValue.equalsIgnoreCase("SC")) {
			...
			attributes.setFields(values.getProperty("autoFill.fl.sc")); 
			//System.out.println("Field list -"+attributes.getFields());

			attributes.setFacetValue(values.getProperty("autoFill.sc.facets"));
			//System.out.println("Facet value -"+attributes.getFacetValue());

			attributes.setFacet(Boolean.TRUE);
			//System.out.println("Facet value -"+attributes.getFacet());

			attributes.setFilterType(values.getProperty("filter.type.all")+":"+values.getProperty("autoFill.sc.filter.type"));
		...
		if(!searchValue.isEmpty()&& searchValue.equalsIgnoreCase("KB")) {
			...
			attributes.setFields(values.getProperty("autoFill.fl.kb")); 
			//System.out.println("Field list -"+attributes.getFields());

			attributes.setFacetValue(values.getProperty("autoFill.kb.facets"));
			//System.out.println("Facet value -"+attributes.getFacetValue());

			attributes.setFacet(Boolean.FALSE);
			//System.out.println("Facet value -"+attributes.getFacet());

			attributes.setFilterType(values.getProperty("filter.type.all")+":"+values.getProperty("autoFill.kb.filter.type"));
			//System.out.println("Facet value -"+attributes.getFilterType());
		...
		if(!searchValue.isEmpty()&& searchValue.equalsIgnoreCase("UD")) {
			...
			attributes.setFilterType(values.getProperty("filter.type.all")+":"+values.getProperty("autoFill.ud.filter.type"));
			//System.out.println("Facet value -"+attributes.getFilterType());
		...
		if(!searchValue.isEmpty()&& searchValue.equalsIgnoreCase("LOC")) {
			...
			attributes.setFilterType(values.getProperty("filter.type.all")+":"+values.getProperty("autoFill.loc.filter.type"));
			//System.out.println("Facet value -"+attributes.getFilterType());
...
public String getAutoQuerySearch(RequestDTO requestDTO, String user, AttributesDTO attributes) throws IOException, SolrServerException {
	 SolrjClientConnectService solrjClientConnectService= new SolrjClientConnectService();

	  String queryResponse=solrjClientConnectService.autoFillQuerySearch(requestDTO.getQueryParam(),requestDTO.getRows(),user,attributes);
